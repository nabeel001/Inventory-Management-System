...
{
    public partial class AvailableReport : Form
    {
        ...
        private void AvailableReport_Load(object sender, EventArgs e)
        {
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("EXEC Report @status = 1", con);
                DataTable dt1 = new DataTable();
                sda1.Fill(dt1);
                label3.Text = dt1.Rows[0][0].ToString();
                label5.Text = dt1.Rows[0][1].ToString();
            }
            else
            {
                label3.Text = "0";
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("select p.p_code, p.p_name, p.p_brand, s.p_qty, s.date, s.p_status from Products p inner join Stock s on p.p_code = s.p_code and s.p_status = 1 ", con);
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                dataGridView1.Rows[n].Cells[6].Value = "Available";
            }
        }
    }
}
